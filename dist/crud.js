...
async function obtenerFilas(cliente, tabla, _, res) {
    const filas = await buscarTodosEnTabla(cliente, tabla);
    console.log(filas);
    res.json(filas);
}
async function agregarFila(cliente, tabla, req, res) {
    const fila = req.body;
    await actualizarTablasJSON(cliente, tabla, [fila]);
    res.json("OK");
}
async function borrarFila(cliente, tabla, req, res) {
    await borrarFilaDeLaTabla(cliente, tabla, req.params);
    res.json("OK");
}
async function editarFila(cliente, tabla, req, res) {
    const fila = req.body;
    console.log(req.body, tabla);
    await editarFilaDeTabla(cliente, tabla, fila);
    res.json("OK");
}
function generarTable(atributos) {
    return atributos.map((atributo) => `<th>${atributo.replace("_", " ")} <button onclick="ordenarPor('${atributo}')">↓</button></th>`)
        .join(`\n`);
}
async function generarForm(cliente, atributos, tabla) {
    const res = await Promise.all(atributos.map(async (atributo) => {
...
async function atenderPedido(respuesta, tabla, req, res) {
...
export async function generarCRUD(app) {
