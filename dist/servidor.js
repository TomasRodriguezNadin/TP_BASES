...
const HTML_PEDIR_ESCRITURA = `<!doctype html>
<html>
    <head>
        <meta charset="utf8">
    </head>
...
`;
function generarMenuEscrituras(parametros, caso) {
...
async function enviarHTMLEscritura(cliente, req, res) {
    const html = await pedirEscrituras(cliente, req.params);
    res.send(html[0]);
}
async function enviarHTMLEscrituras(cliente, req, res) {
    const html = await pedirEscrituras(cliente, req.params);
    res.send(html.join(`\n`));
}
async function cargarJSON(cliente, req, _) {
...
async function atenderPedido(respuesta, mensajeError, req, res) {
